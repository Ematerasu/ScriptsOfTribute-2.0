...
namespace Bots
{
    public static class HandTierList
    {
        ...
        static readonly Dictionary<CardId, int> handTierDict = new Dictionary<CardId, int> {
            ...
            { CardId.ALESSIAN_REBEL, CURSE},

            { CardId.AYLEID_DEFECTOR,          B },
            { CardId.AYLEID_QUARTERMASTER,     B },
            { CardId.CHAINBREAKER_CAPTAIN,     A },
            { CardId.CHAINBREAKER_SERGEANT,    B },
            { CardId.MORIHAUS_SACRED_BULL,     S },
            { CardId.MORIHAUS_THE_ARCHER,      A },
            { CardId.PELINAL_WHITESTRAKE,      S },
            { CardId.PRIESTESS_OF_THE_EIGHT,   B },
            { CardId.SAINTS_WRATH,             B },
            { CardId.SOLDIER_OF_THE_EMPIRE,    C },
            { CardId.WHITESTRAKE_ASCENDANT,    S },
        };

        public static int GetCardTier(CardId cardId)
        {
            if (handTierDict.TryGetValue(cardId, out int tier))
                return tier;
